...
function generaColumnas( $info, $json ){
	...
	if( $estatus == 'reestablecida' && $correo != "" && $codigo != "" ){
...
function mailCodigo($para, $codigo, $json){
    ...
    $to = $para; // para
    $from = "admin@siafut.com"; // De quien
    $subjet = "SOLUCIONAFORE - clave de acceso temporal al sistema"; // Asunto
    $message = "Hola<br><br>La clave temporal para tener acceso al sistema es ".$codigo.", posteriormente deberas actualizarla.<br><br>Saludos "; 
    $replyTo = "no-reply"; 
    $LE  = "\r\n"; 
    ...
    try{
		...
		$json .= "\""."claveTemporal"."\":\""."enviada"."\"";
	
    } catch (Exception $e) {
    	if($json[strlen($json)-1] != "{"){
			$json .= ",";
		}
		$json .= "\""."claveTemporal"."\":\""."error"."\"";
    }
