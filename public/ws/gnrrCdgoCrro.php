...
try{ // Connect to server and select databse.
	...
	$myFile = "../txt/sql/gnrrCdgoCrro.sql";
...
function generaColumnas( $info, $json ){
	...
	if( $estatus == 'actualizado' && $correo != "" && $codigo != "" ){
	    $json = mailCodigo($correo,$codigo,$json);
	}
    $json .= "}";
    return $json;
}

function mailCodigo($para, $codigo, $json){
    //Envio de correo para confirmar email
    $to = $para; // para
    $from = "admin@solucionafore.com"; // De quien
    $subjet = "SOLUCIÓN AFORE confirma tu correo electronico"; // Asunto
    $message = "Hola<br><br>El codigo de verificación para confirmar tu correo electrónico es ".$codigo.", por favor inicia sesión en el sistema y proporciona el código para finalizar tu registro y tener acceso al sistema<br><br>Saludos"; // mensaje
    $replyTo = "no-reply"; 
    $LE  = "\r\n"; 
