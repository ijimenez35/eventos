            ...
            if ($prev && $prev[1][0] != ';') {
                ...
                foreach ($sqlPart as $part) {
                    if (trim($part)) { // no empty queries
                        ...
                        if( $result = mysqli_query($mysqli, $part) ){
                    	    ...
                    	    //echo "<br>- Este no se ejecuta ->".$part;
                    	    //$mysqli->query($part);
                    	}
                    }
                }
...
function generaColumnas( $info, $json ){
	...
	if( ($estatus == 'insertado' || $estatus ==  'actualizado') && $correo != "" ){
	    $json = mailAltaUsuario($correo,$json);
	}
    $json .= "}";
    return $json; 
}

function mailAltaUsuario($para, $json){
    //Envio de correo para confirmar email
    $to = $para; // para
    $from = "admin@solucionafore.com"; // De quien
    $subjet = "SOLUCION AFORE registro confirmado"; // Asunto
    $message = "Su registro en el sistema ha sido confirmado con el correo <b>".$para."</b>, por favor contacte al administrador para que este confirme su registro y asi finalizar su registro."; // mensaje
